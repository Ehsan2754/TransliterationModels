...
def gen_validation_data(p, data, seq_len, transliteration, trans_vocab_size, trans_to_index):
    ...
    if new_p != len(data):
        if max([raw_translit.rfind(u' '), raw_translit.rfind(u'\t'), raw_translit.rfind(u'\n')]) > 0:
            new_p = max([raw_translit.rfind(u' '), raw_translit.rfind(
                u'\t'), raw_translit.rfind(u'\n')])
...
def translate_romanized(predict, data, seq_len, transliteration, trans_vocab_size, trans_to_index, index_to_char, long_letter_reverse_mapping):
    ...
    sentence_out = "\n"
    while not turned:
        x, non_valids, p, turned = gen_validation_data(
            p, data, seq_len, transliteration, trans_vocab_size, trans_to_index)
        guess = utils.one_hot_matrix_to_sentence(
            predict(x)[0], index_to_char).replace(u'\u2001', '').replace(u'\u2000', '')
...
def test(predict, data, language, model_name, seq_len, batch_size, long_letter_reverse_mapping, transliteration, trans_to_index, char_to_index, index_to_trans, index_to_char):
    ...
    for ((x_list, y_list, indices, delimiters), non_valids_list) in utils.data_generator(data, seq_len, batch_size, transliteration, trans_to_index, char_to_index, is_train=False):
        ...
        for (x, y, guess, non_valids, index, delimiter) in zip(x_list, y_list, guess_list, non_valids_list, indices, delimiters):
            ...
            guess = utils.one_hot_matrix_to_sentence(
                guess, index_to_char).replace(u'\u2001', '').replace(u'\u2000', '')
            ...
            sentences.append(
                (index, sentence_in, sentence_real, final_guess, delimiter))
    ...
    for (i, j) in zip(sentence_real.split('\n'), sentence_out.split('\n')):
        tmp = [(tmp_i, tmp_j) for (tmp_i, tmp_j) in zip(i.split(' '), j.split(
            ' ')) if len(tmp_i) > 0 and not (tmp_i[0] >= u'Ա' and tmp_i[0] <= u'Ֆ')]
        if len(tmp) > 0:
            i_tmp, j_tmp = zip(*tmp)
            i_tmp = ' '.join(i_tmp)
            j_tmp = ' '.join(j_tmp)
    ...
    print("Writing to -> " + 'languages/' + language +
          '/results.' + model_name.split('/')[-1])

    fl = codecs.open('languages/' + language + '/results.' +
                     model_name.split('/')[-1], 'w', encoding='utf-8')
    fl.write(sentence_in + '\n' + sentence_real + '\n' + sentence_out + '\n')
    fl.write(str(distance) + ' / ' + str(len(sentence_real)) +
             "  " + str(len(sentence_out)))
...
def main():
    ...
    (output_layer, predict) = utils.define_model(args.hdim, args.depth,
                                                 trans_vocab_size=trans_vocab_size, vocab_size=vocab_size, is_train=False)
    if args.model:
    ...
    if args.translit_path:
        ...
        translate_romanized(predict, data, args.seq_len, trans, trans_vocab_size,
                            trans_to_index, index_to_char, long_letter_reverse_mapping)

    else:
        test(predict, test_text, args.language, args.model, args.seq_len, args.batch_size,
             long_letter_reverse_mapping, trans, trans_to_index, char_to_index, index_to_trans, index_to_char)
